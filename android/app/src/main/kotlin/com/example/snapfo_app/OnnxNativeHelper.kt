...
class OnnxNativeHelper(context: Context) {
    ...
    fun loadModel(key: String, modelBytes: ByteArray) {
        try {
            val sessionOptions = OrtSession.SessionOptions()
            sessions[key] = env.createSession(modelBytes, sessionOptions)
            println("Model $key loaded successfully from memory")
        } catch (e: Exception) {
            throw RuntimeException("Failed to load model $key: ${e.message}", e)
        }
    }
